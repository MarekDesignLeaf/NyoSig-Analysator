#!/usr/bin/env python3
# -*- coding: utf-8-sig -*-
# nyosig_analysator_core_v7.5a -- CORE
...
DEFAULT_PROJECT_ROOT = "/storage/emulated/0/NyoSig/NyoSig_Analysator"
...
def _ohlcv_ingest(con, run_id, snap_key, markets, vs_currency, paths, log_cb,
    ...
    TF_DAYS_MAP = {"15m": 1, "1h": 7, "4h": 14, "1d": 90}
...
def _http_get_json(url, timeout=10, headers=None):
    ...
    _h = {"User-Agent": "NyoSig-Analysator/7.5a"}
    if headers:
        _h.update(headers)
...
class LayerAdapter:
    ...
    SCOPE_KEY = "base"
    VERSION = "v1"
    _config = {}  # v8.0a fix: class-level default so get_config never crashes
